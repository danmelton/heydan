...
describe HeyDan::Base do
  ...
  it 'create_settings_file' do
    ...
    expect(YAML.load(File.read(settings_file))).to eq ({:help=>false, :folders=>{:jurisdictions=>"spec/tmp1/spec/tmp/jurisdictions", :sources=>"spec/tmp1/spec/tmp/sources", :downloads=>"spec/tmp1/spec/tmp/downloads", :datasets=>"spec/tmp1/spec/tmp/datasets"}, :sources=>{:heydan_sources=>"https://github.com/danmelton/heydan_sources.git"}, :elasticsearch=>{:url=>"http://localhost:9200"}})
  end
